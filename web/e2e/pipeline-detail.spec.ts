...
test.describe("Pipeline Detail", () => {
  test("loads typescript_build pipeline detail page", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "typescript_build" }))
      .toBeVisible();
  });
  ...
  test("renders the React Flow DAG canvas", async ({ page }) => {
    ...
    const rfWrapper = page.locator(
      ".react-flow__renderer, .react-flow__viewport, [data-testid='rf__wrapper']",
    );
  ...
  test("shows parallel pipeline DAG correctly (parallel_build)", async ({ page }) => {
    await page.goto("/pipelines/parallel_build/1.0.0");
    await expect(page.getByRole("heading", { name: "parallel_build" }))
      .toBeVisible();
    await page.waitForTimeout(1000);
